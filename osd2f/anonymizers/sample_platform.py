...
async def redact_text(
    ...
    sep_strings = [
        ...
        "posted a comment on the photo by",
        "posted a comment on the video by",
        "posted a reply to"
    ]
    for sep_string in sep_strings:
        if sep_string in entry["title"]:
            ...
            entry["title"] = (
                entry["title"].replace(ego, "<user> ").replace(alter, " <alter>")
            )
            ...
            if "data" in entry:
                for data_item in entry["data"]:
                    ...
                    if comment_comment := data_item.get("comment.comment"):
                        ...
                        comment_comment = comment_comment.replace(
                            alter.strip(), "<alter>"
                        )
    ...
    sep_strings = ["wrote on", "added a new photo to", "was with"]
            ...
            if "data" in entry:
                for data_item in entry["data"]:
                    if reaction_actor := data_item.get("reaction.actor"):
                        ...
                        reaction_actor = reaction_actor.replace(
                            alter.strip(), "<alter>"
                        )
    ...
    sep_strings = [
        ...
        "vindt een bericht leuk." "vindt de video",
    ...
    for sep_string in sep_strings:
        if sep_string in entry["title"]:
            ...
            entry["title"] = (
                entry["title"]
                .replace(ego.strip(), "<user> ")
                .replace(alter.strip(), " <alter>")
            )
    ...
    # Dutch comments
    sep_strings = [
        "heeft gereageerd op het bericht",
